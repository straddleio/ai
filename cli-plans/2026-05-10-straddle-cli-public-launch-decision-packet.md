...
- Local compatibility evidence now exists at `cli-plans/2026-05-10-straddle-cli-current-cli-compatibility-inventory.md`. It records installed Stainless-generated `straddle` version `0.1.0`, source remote `https://github.com/stainless-sdks/straddle-cli`, Stainless README provenance, installed help for `charges create` and `customers list`, preview help for the same commands, and `release plan compatibility --json`. It is local evidence only, states that public `straddle` replacement is not approved, and does not approve any migration or alias plan.
...
- Local packaged-client credential smoke passed on 2026-05-10. The run used `make package-readiness`, packaged CLI help, packaged `setup check`, packaged `credentials plan packaged-client`, expected unauthenticated `auth status`, JSON-RPC `initialize` plus `tools/list` against `dist/local/straddle-pp-mcp`, and `make clean`. It used no credentials, made no Straddle API calls, and executed no MCP tools.
...
- Credential launch posture remains unapproved for broad public launch. `credentials plan launch` reports that keychain-backed storage exists as opt-in preview support. `credentials plan packaged-client` makes local built-binary smoke commands machine-readable for `dist/local/straddle-pp-cli` and `dist/local/straddle-pp-mcp`, including JSON-RPC `tools/list` only, and the local packaged-client smoke passed. Broad launch still needs owner/security approval, approved live read-only smoke, approved public docs wording, signed/notarized packaging posture, and desktop MCP packaging posture.
...
<td>Current CLI compatibility inventory, `release plan compatibility --json` output, installed `straddle` help and version output, Stainless source-tree markers, command collision review, migration or alias plan, public help output review, and product approval for the name.</td>
...
<td>Safe stdin, opt-in keychain support, machine-readable packaged-client local smoke steps, and the passed local packaged-client smoke reduce exposure. Broad public launch still needs owner/security approval and approved live read-only smoke.</td>
<td>Owner/security decision, threat model for local config, keychain, and MCP launches, token redaction checks, no argv token paths in examples, approved live read-only smoke, approved public docs wording, signed/notarized packaging posture, and desktop MCP packaging posture.</td>
