...
The reviewed evidence says Printing Press `4.2.0` generated `straddle-pp-cli` and `straddle-pp-mcp` from the Straddle OpenAPI source. The latest completion audit records passing local validation, passing Go tests, credential-free CLI smoke, and credential-free MCP `tools/list` smoke with `tool_count: 87`. It also records no public publish, no approved live read-only smoke, no signed or notarized macOS distribution, no desktop MCP package, and no final launch decision for credential storage. `credentials plan packaged-client` now makes the local built-binary smoke commands machine-readable, but it does not run smoke, execute MCP tools, or approve launch.
...
- Credential launch posture is still not approved for broad public launch. Keychain-backed storage exists as opt-in preview support, and packaged-client smoke steps are now machine-readable local guidance, but packaged-client smoke is only planned or local evidence until run and reviewed. Owner/security approval, approved live read-only smoke, public docs wording, signed/notarized packaging posture, and desktop MCP packaging posture are still missing.
...
- Credential posture: pass requires owner/security approval for the chosen storage posture, reviewed packaged-client smoke, approved live read-only smoke, public docs wording, signed/notarized packaging posture, desktop MCP packaging posture, and redaction checks. Current result: fail. Approval remains open.
