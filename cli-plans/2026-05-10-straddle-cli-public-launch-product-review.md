...
- Current CLI compatibility inventory: `cli-plans/2026-05-10-straddle-cli-current-cli-compatibility-inventory.md`.
...
The reviewed evidence says Printing Press `4.2.0` generated `straddle-pp-cli` and `straddle-pp-mcp` from the Straddle OpenAPI source. The latest completion audit records passing local validation, passing Go tests, credential-free CLI smoke, credential-free MCP `tools/list` smoke with `tool_count: 87`, and a no-secret packaged-client credential smoke against `dist/local/straddle-pp-cli` and `dist/local/straddle-pp-mcp`. It also records no public publish, no approved live read-only smoke, no signed or notarized macOS distribution, no desktop MCP package, and no final launch decision for credential storage. `credentials plan packaged-client` makes the local built-binary smoke commands machine-readable, and the local smoke passed, but that does not execute MCP tools or approve launch.

The latest local surfaces reduce planning ambiguity but do not approve launch. `cli-plans/2026-05-10-straddle-cli-current-cli-compatibility-inventory.md` records local compatibility evidence for the installed Stainless-generated `straddle` command and the Printing Press preview, including installed version `0.1.0`, source repo evidence, overlapping `charges create` and `customers list` help, preview-only local planning features, and migration implications. `release plan compatibility --json` still records that public `straddle` replacement is not approved and does not approve any migration or alias plan. `release plan naming --json` records that `straddle-pp-cli` is still the preview command and that the public `straddle` binary is not approved. `release plan docs-support --json` records that public docs and support are not approved yet and that public docs must not imply workflow execution, production readiness, or replacement of public `straddle`. `release plan owner-decisions --json` records the owner choices still required for public binary name, first release channel, signing, desktop MCP packaging, credential posture, live smoke, docs/support scope, and workflow execution claims, and states that no owner has approved those launch choices. `smoke plan approval --json` records the approval packet fields, expected evidence, stop criteria, and redaction guidance required before any future live read-only smoke.
...
- `about`, `setup check`, `docs search`, `sandbox guide`, `ops guide`, `workflow plan`, `smoke plan`, `release plan`, and `credentials plan` are documented as local or planning surfaces. The `packaged-client` credentials surface is local-only guidance for `dist/local/straddle-pp-cli` and `dist/local/straddle-pp-mcp`, including JSON-RPC `tools/list` only, and the local no-secret packaged-client smoke passed.
- The current CLI compatibility inventory plus `release plan compatibility --json`, `release plan naming --json`, `release plan docs-support --json`, `release plan owner-decisions --json`, and `smoke plan approval --json` provide local evidence for the remaining current-CLI compatibility, naming, docs/support, owner-decision, and live-smoke approval decisions without choosing the public command, running smoke, publishing, signing, notarizing, approving support docs, or approving workflow execution claims.
...
- Credential launch posture is still not approved for broad public launch. Keychain-backed storage exists as opt-in preview support, packaged-client smoke steps are machine-readable local guidance, and local packaged-client smoke has passed. That is not enough for public launch. Owner/security approval, approved live read-only smoke, public docs wording, signed/notarized packaging posture, and desktop MCP packaging posture are still missing.
...
- Command name: pass requires owner decision for public binary name, compatibility review against the installed `straddle` command and Stainless source tree, collision review, migration or alias plan if using `straddle`, and approved public help output. Current result: fail. Current compatibility evidence improved through the local inventory, but the preview name is still `straddle-pp-cli` and the public `straddle` decision is open.
...
- Credential posture: pass requires owner/security approval for the chosen storage posture, approved live read-only smoke, public docs wording, signed/notarized packaging posture, desktop MCP packaging posture, and redaction checks. Current result: fail. Local packaged-client smoke passed, but approval remains open.
