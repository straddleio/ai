...
- MCP count evidence is now resolved. `packages/cli/straddle-pp-cli/.printing-press.json:15` records 70 generated endpoint tools, `packages/cli/straddle-pp-cli/internal/mcp/tools.go` has 73 typed tools after adding `search`, `sql`, and `context`, and runtime `tools/list` expects 87 tools after fourteen Cobra shell-out tools are registered, including `credentials_plan`, `docs_search`, `ops_guide`, `release_plan`, `sandbox_guide`, `setup_check`, `smoke_plan`, and `workflow_plan`. Phase 4 now validates this source count invariant.
...
- `workflow` currently has `archive`, `plan`, and `status`. `workflow plan` is local-only structured command planning for reconciliation, fraud monitoring, collections, reporting, monitoring, and all. Evidence: `packages/cli/straddle-pp-cli/internal/cli/channel_workflow.go`.
...
| Use `doctor`, `auth`, `profile`, `which`, `agent-context`, `sync`, `search`, `analytics`, `tail`, and `workflow` before inventing new support commands | required | These support roots already exist and cover much of the benchmark surface. The bounded exception is `setup check`, a local-only readiness summary that does not replace `doctor` and does not make live calls. |
...
- Generated CLI metadata and typed MCP registrations now have explicit count semantics: generated metadata says 70 endpoint tools, typed registration count is 73 after 3 framework typed tools, and runtime `tools/list` count is 87 after fourteen Cobra shell-out tools.
