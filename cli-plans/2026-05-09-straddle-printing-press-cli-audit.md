...
Safe token guidance is done for the current preview docs: stdin auth is documented, environment injection is documented, docs say not to commit, print, log, or pass tokens through argv, and `credentials plan [surface]` documents the remaining launch decision. No OS keychain or secure-store implementation exists yet; launch needs an explicit decision to approve env/config-only for first public release or implement OS secure storage. The agent envelope is now covered for generated list/read commands, `printJSONFiltered` local helpers under `--agent`, `agent-context --agent`, `about --agent`, `sync --agent`, and real `tail --agent`.
