...
1. Finish the broader CLI contract and launch path. The current honesty docs slice is partial and does not complete the public CLI goal.
   ...
   - Release plan is implemented as local-only guidance for public-release readiness across archives, docs/support, Homebrew, MCP, naming, npm, and signing. It reduces the distribution, docs, and support decision blockers but does not publish, upload, push, sign, notarize, call external services, execute MCP tools, read secrets, approve public docs, approve support, or approve launch.
