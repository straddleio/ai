...
`shipcheck local` is an executable local verifier for this preview. It returns `command`, `passed`, `checks`, and `safety`. It checks required public-preview helper commands, explicit goal surfaces, workflow plans for `reconciliation`, `fraud-monitoring`, `collections`, `reporting`, and `monitoring`, `docs search --source commands` coverage for helper commands such as `workflow plan`, `mcp config`, `mcp bundle`, and `benchmark ramp`, and Printing Press/OpenAPI provenance from `.printing-press.json`. The goal surface check covers local word art presentation, agent output, setup, customers, payments, reconciliation data, fraud review, collections, reporting, monitoring, sandbox testing, docs search, MCP config, Ramp benchmark, and release readiness. The provenance check records the generator version, Straddle docs OpenAPI source path, and source checksum evidence. It rehashes the source OpenAPI document when that file is available, but clean clones and packaged `dist/local` directories can pass with the recorded checksum when the local source path is absent. Set `STRADDLE_PP_SHIPCHECK_STRICT_PROVENANCE=1` to make missing source files fail. It also verifies `mcp_binary` is `straddle-pp-mcp`, and packaged metadata next to `straddle-pp-cli` requires the `straddle-pp-mcp` sibling to exist. `spec.json` is generated and transformed, while `.printing-press.json` preserves the checksum evidence for the source OpenAPI document. With `--mcp-binary`, it runs a local child process and sends only stdio MCP `initialize` plus `tools/list`, requiring `mcp_config`, `mcp_bundle`, `docs_search`, `workflow_plan`, `release_plan`, `credentials_plan`, `smoke_run`, `setup_check`, `shipcheck_local`, and `shipcheck_public`. It rejects `--deliver`, avoids secret-bearing child env vars, and does not publish, sign, notarize, or write production. It does not sandbox the child MCP process or prove that child process behavior beyond the MCP response.

`shipcheck public` is the executable final-launch gate. It includes the full local preview result under `local_preview`, returns `local_preview_passed`, and keeps `ready` false until public launch approvals are recorded. The failed checks are intentional blockers for owner approval, public npm/npx/Homebrew/GitHub distribution, docs and support ownership, desktop MCP packaging, live read-only smoke approval, and signing or notarization posture.
