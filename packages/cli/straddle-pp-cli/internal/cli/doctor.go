...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  straddle-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.Getenv("STRADDLE_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "STRADDLE_TOKEN")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "STRADDLE_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "straddle-pp-cli"
