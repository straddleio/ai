...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("STRADDLE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("STRADDLE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "straddle-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.straddle-pp-cli/feedback.jsonl.
When ` + "`STRADDLE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`STRADDLE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  straddle-pp-cli feedback list
  straddle-pp-cli feedback list --limit 5
  straddle-pp-cli feedback list --json`,
