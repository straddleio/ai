---
name: pp-straddle
description: "Printing Press CLI for Straddle. Postman environment details omitted from generated CLI artifacts."
author: "hello-keith"
license: "Apache-2.0"
...
For local packaging readiness, build both preview binaries into `dist/local/`, verify they exist, run packaged `shipcheck local` against the packaged MCP sibling, and verify packaged `shipcheck public` still blocks public launch while proving local preview passed:
...
`make release-readiness` is the canonical non-publishing release gate. It runs Go tests, local package-readiness, GoReleaser config validation, GoReleaser snapshot archive generation, archive and checksum verification, local cask binary checks, current-platform archive extraction, archived `shipcheck local`, archived `shipcheck public` blocker verification, archived `mcp bundle`, and local npm package assembly plus verification. The npm verification checks private package metadata, bundled vendor binaries, current-platform CLI and MCP wrapper smokes, packaged `shipcheck public` blocker verification, token-shaped strings, publishing-command strings, `npm pack --dry-run`, and offline local tarball execution. The archives include `straddle-pp-cli`, `straddle-pp-mcp`, `.printing-press.json`, `manifest.json`, `README.md`, and `LICENSE`; they are local `dist/` artifacts only. `make release-check` and `make release-snapshot` remain lower-level troubleshooting targets. `make clean` removes the generated output after inspection.

`make completion-audit` is the canonical local objective audit. It runs `make release-readiness`, then verifies the packaged current-platform CLI and MCP sibling against the prompt surfaces: Printing Press/OpenAPI provenance, Ramp benchmark, local docs command search, workflow plans for reconciliation, fraud monitoring, collections, reporting, and monitoring, local MCP `tools/list` smoke, approval template shape, public shipcheck blockers, and redacted release-candidate approval validation. It requires a clean tracked worktree and still does not publish, upload, push, call Straddle APIs, call docs endpoints, read secrets, execute credential-bearing MCP tools, sign, notarize, or write production.

The manual GitHub Actions workflow `.github/workflows/cli-release-candidate.yml` turns the same local proof into a review artifact. It accepts a strict `shipcheck public --approval-file` JSON record as raw JSON or base64 JSON, accepts an optional `candidate_ref` so a default-branch workflow can review a candidate branch, runs plugin validation, generated CLI validation, Go tests, `make release-readiness`, validates the approval against the built CLI, and uploads `dist/` plus redacted release-candidate metadata for review. It writes the raw approval JSON and raw shipcheck evidence only to a temporary runner path, so the artifact does not include approval contents. It is still non-publishing: no release creation, npm publish, Homebrew publish, signing, notarization, Straddle API calls, credential-bearing MCP tool calls, or secret reads.
...
`shipcheck local` verifies the in-process CLI command tree, explicit goal surfaces, required workflow plan names, local docs command-search helper indexing, Printing Press/OpenAPI provenance, and safety metadata. The explicit goal surface check covers local word art presentation, agent output, setup, customers, payments, reconciliation data, fraud review, collections, reporting, monitoring, sandbox testing, docs search, MCP config, Ramp benchmark, and release readiness. The provenance check reads `.printing-press.json`, requires `printing_press_version`, the Straddle docs OpenAPI source path, `spec_checksum`, and `mcp_binary: "straddle-pp-mcp"`, then hashes the referenced source OpenAPI file when it exists locally. If that source file is unavailable in a clean clone or packaged `dist/local` directory, shipcheck passes with recorded checksum evidence; set `STRADDLE_PP_SHIPCHECK_STRICT_PROVENANCE=1` when you want missing source files to fail. When `.printing-press.json` sits next to a packaged `straddle-pp-cli`, shipcheck also requires the `straddle-pp-mcp` sibling to exist. `spec.json` is generated and transformed, so shipcheck does not require it to match the source checksum. With `--mcp-binary`, it runs a local child process and sends only stdio JSON-RPC `initialize` plus `tools/list`, requiring `mcp_config`, `mcp_bundle`, `docs_search`, `workflow_plan`, `release_plan`, `credentials_plan`, `smoke_run`, `setup_check`, `shipcheck_local`, and `shipcheck_public`. It rejects `--deliver` and token literals in `--mcp-binary`, and the child process receives an allowlisted environment. It does not publish, sign, notarize, or write production. It does not sandbox the child MCP process or prove that child process behavior beyond the MCP response.

`shipcheck public` runs the local preview verifier and then fails on the public launch approvals that are still external to this repo: owner approval, public distribution, docs and support ownership, desktop MCP packaging, live read-only smoke approval, and signing or notarization posture. With `--approval-template`, it prints only the strict JSON approval-file template for the current clean local preview commit, with empty reviewer fields, false approval booleans, and empty evidence values for all six approval keys. The template path does not run local preview, publish, call APIs, execute MCP, read secrets, sign, notarize, or write production. With `--approval-file <path>`, it validates one strict local JSON approval record containing `approver`, RFC3339 `approved_at`, the current `local_preview_commit`, true values for every public approval field, and non-empty evidence for each blocker. Approval-file validation is local only; it can clear blocker checks only when the local preview already passes, and it still does not publish, push, upload, call Straddle APIs, call docs endpoints, call GitHub, call npm, call Homebrew, execute MCP tools, read secrets, sign, notarize, or write production. Without `--approval-file`, the command keeps the blockers in place and returns a failure code while emitting JSON or the target agent envelope.
...
- `straddle-pp-cli account-settings <account_id>` - Get all resolved settings for the specified account, including inherited values from organization, platform, and...
...
- `straddle-pp-cli accounts create` - Creates a new account associated with your Straddle platform integration. This endpoint allows you to set up an...
- `straddle-pp-cli accounts get` - Retrieves the details of an account that has previously been created. Supply the unique account ID that was returned...
- `straddle-pp-cli accounts list` - Returns a list of accounts associated with your Straddle platform integration. The accounts are returned sorted by...
- `straddle-pp-cli accounts update` - Updates an existing account's information. This endpoint allows you to update various account details during...
...
- `straddle-pp-cli bridge create` - Creates a new paykey using a Quiltt token as the source. This endpoint allows you to create a secure payment token...
- `straddle-pp-cli bridge create-bank-account-paykey` - Use Bridge to create a new paykey using a bank routing and account number as the source. This endpoint allows you to...
- `straddle-pp-cli bridge create-plaid-paykey` - Use Bridge to create a new paykey using a Plaid token as the source. This endpoint allows you to create a secure...
- `straddle-pp-cli bridge create-speedchex` - Creates a new paykey using a Speedchex token as the source. This endpoint allows you to create a secure payment...
...
- `straddle-pp-cli charges get` - Retrieves the details of an existing charge. Supply the unique charge `id`, and Straddle will return the...
- `straddle-pp-cli charges update` - Change the values of parameters associated with a charge prior to processing. The status of the charge must be...
...
- `straddle-pp-cli customers create` - Creates a new customer record and automatically initiates identity, fraud, and risk assessment scores. This endpoint...
- `straddle-pp-cli customers delete` - Permanently removes a customer record from Straddle. This action cannot be undone and should only be used to satisfy...
- `straddle-pp-cli customers get` - Retrieves the details of an existing customer. Supply the unique customer ID that was returned from your 'create...
- `straddle-pp-cli customers list` - Lists or searches customers connected to your account. All supported query parameters are optional. If none are...
- `straddle-pp-cli customers update` - Updates an existing customer's information. This endpoint allows you to modify the customer's contact details, PII,...
...
- `straddle-pp-cli funding-events get` - Retrieves the details of an existing funding event. Supply the unique funding event `id`, and Straddle will return...
...
- `straddle-pp-cli linked-bank-accounts create` - Creates a new linked bank account associated with a Straddle account. This endpoint allows you to associate external...
- `straddle-pp-cli linked-bank-accounts get` - Retrieves the details of a linked bank account that has previously been created. Supply the unique linked bank...
- `straddle-pp-cli linked-bank-accounts list` - Returns a list of bank accounts associated with a specific Straddle account. The linked bank accounts are returned...
- `straddle-pp-cli linked-bank-accounts update` - Updates an existing linked bank account's information. This can be used to update account details during onboarding...
...
- `straddle-pp-cli organizations create` - Creates a new organization related to your Straddle integration. Organizations can be used to group related accounts...
- `straddle-pp-cli organizations get-by-id` - Retrieves the details of an Organization that has previously been created. Supply the unique organization ID that...
- `straddle-pp-cli organizations list` - Retrieves a list of organizations associated with your Straddle integration. The organizations are returned sorted...
...
- `straddle-pp-cli paykeys get` - Retrieves the details of an existing paykey. Supply the unique paykey `id` and Straddle will return the...
- `straddle-pp-cli paykeys list` - Returns a list of paykeys associated with a Straddle account. This endpoint supports advanced sorting and filtering...

**payments** - Payments provide endpoints to filter both Charges and Payouts with multiple different parameters.

- `straddle-pp-cli payments` - Search for payments, including `charges` and `payouts`, using a variety of criteria. This endpoint supports advanced...
...
- `straddle-pp-cli payouts get` - Retrieves the details of an existing payout. Supply the unique payout `id` to retrieve the corresponding payout...
- `straddle-pp-cli payouts update` - Update the details of a payout prior to processing. The status of the payout must be `created`, `scheduled`, or...
...
- `straddle-pp-cli representatives create` - Creates a new representative associated with an account. Representatives are individuals who have legal authority or...
- `straddle-pp-cli representatives get` - Retrieves the details of an existing representative. Supply the unique representative ID, and Straddle will return...
- `straddle-pp-cli representatives list` - Returns a list of representatives associated with a specific account or organization. The representatives are...
- `straddle-pp-cli representatives update` - Updates an existing representative's information. This can be used to update personal details, contact information,...
...
```bash
straddle-pp-cli which "<capability in your own words>"
```
...
Use `smoke run <scope> --approval-file <path>` only after a written approval file exists. The executable slice supports `setup`, `customers`, and `mcp`. It rejects production-looking URLs, scope mismatches, token literals, credential-bearing URL parts, and unsafe MCP binary strings. `setup` remains local-only. `customers` makes one read-only `GET /v1/customers` request to the approved base URL. `mcp` requires approval file `allowed_scope: "mcp"` and runs only a local stdio JSON-RPC initialize plus `tools/list` against `--mcp-binary`, default `straddle-pp-mcp`; it requires `mcp_config` and `smoke_run` to be present and records the tool count in the transcript. The run writes the redacted transcript artifact to the approval file `transcript_path`, creating parent directories as needed. It attaches caller-supplied `STRADDLE_TOKEN` only for trusted `customers` runs. MCP smoke does not read secrets, pass `STRADDLE_TOKEN`, call Straddle APIs, execute credential-bearing MCP tools, or approve launch.
...
- **Filterable** - `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  straddle-pp-cli accounts list --agent --select id,name,status
  ```
...
```
straddle-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
straddle-pp-cli feedback --stdin < notes.txt
straddle-pp-cli feedback list --json --limit 10
```

Entries are stored locally at `~/.straddle-pp-cli/feedback.jsonl`. They are never POSTed unless `STRADDLE_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `STRADDLE_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
straddle-pp-cli profile save briefing --json
straddle-pp-cli --profile briefing accounts list
straddle-pp-cli profile list --json
straddle-pp-cli profile show briefing
straddle-pp-cli profile delete briefing --yes
```
...
1. Check if installed: `which straddle-pp-cli`
   If not found, offer to install (see Prerequisites at the top of this skill).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   straddle-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `straddle-pp-cli <command> --help`.
