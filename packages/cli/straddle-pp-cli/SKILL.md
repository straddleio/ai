...
`make release-readiness` is the canonical non-publishing release gate. It runs Go tests, local package-readiness, GoReleaser config validation, GoReleaser snapshot archive generation, archive and checksum verification, local cask binary checks, current-platform archive extraction, archived `shipcheck local`, and archived `mcp bundle`. The archives include `straddle-pp-cli`, `straddle-pp-mcp`, `.printing-press.json`, `manifest.json`, `README.md`, and `LICENSE`; they are local `dist/` artifacts only. `make release-check` and `make release-snapshot` remain lower-level troubleshooting targets. `make clean` removes the generated output after inspection.
