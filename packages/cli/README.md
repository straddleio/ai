...
- Public-release readiness now has the local-only `straddle-pp-cli release plan [surface]` terminal flow. It reduces the distribution, compatibility, docs, and support decision blockers by making archive, current-CLI compatibility, docs/support, Homebrew, MCP, naming, npm, and signing decisions explicit, but it does not publish, approve launch, replace `straddle`, approve public docs, or approve support.
