...
Use `straddle-pp-cli shipcheck public --approval-template` to print the strict approval JSON shape for the current clean local preview commit before filling an approval file. The template command does not run local preview, publish, call APIs, execute MCP, read secrets, sign, notarize, or write production.
...
`straddle-pp-cli smoke run <scope> --approval-file <path>` is the approval-file-gated executable smoke surface. This slice supports `setup`, `customers`, and `mcp`. It rejects missing approval, scope mismatch, production-looking base URLs, URL credentials, URL queries or fragments, and token literals in the approval file. `setup` stays local-only. `customers` performs one read-only `GET /v1/customers` request against the approved base URL. `mcp` runs only a local stdio JSON-RPC initialize plus `tools/list` smoke against the generated `straddle-pp-mcp` sibling, without reading secrets, passing `STRADDLE_TOKEN`, calling Straddle APIs, or executing credential-bearing MCP tools. The run writes the redacted transcript artifact to the approval file `transcript_path`, creating parent directories as needed. It attaches caller-supplied `STRADDLE_TOKEN` only when the approval file names `env:STRADDLE_TOKEN` and the target is trusted, such as `sandbox.straddle.com` or a local test server.
...
`straddle-pp-cli shipcheck public --approval-file <path>` is the strict local final gate after owners have approved launch. Generate the starting shape with `straddle-pp-cli shipcheck public --approval-template`, then fill `approver`, RFC3339 `approved_at`, true values for `owner_approval`, `public_distribution`, `docs_support`, `desktop_mcp`, `live_smoke`, and `signing_notarization`, plus non-empty evidence strings for each approval. The template includes the current clean `local_preview_commit` and leaves approval values false. Approval-file validation validates only; it does not publish, upload, call external services, execute MCP tools, read secrets, sign, notarize, or write production.
